...
class Eval_thread():
    ...
    def run(self):
        ...
        self.LOG('#[{:10} Dataset] [{:6} Method]# [{:.4f} mae], [{:.4f} max-fmeasure], [{:.4f} mean-fmeasure], [{:.4f} adp-fmeasure], [{:.4f} max-Emeasure], [{:.4f} mean-Emeasure], [{:.4f} adp-Emeasure], '
                 '[{:.4f} S-measure_alpha05].\n'
                 .format(self.dataset, self.method, mae, max_f, mean_f, adp_f, max_e, mean_e, adp_e, s_alpha05))

        return '[cost:{:.4f}s][{:6} Dataset] [{:6} Method] \n{:.4f},  mae, \n{:.4f},  max-fmeasure, \n{:.4f},  mean-fmeasure, \n{:.4f},  adp-fmeasure, \n{:.4f},  max-Emeasure,' \
               ' \n{:.4f},  mean-Emeasure, \n{:.4f},  adp-Emeasure, \n{:.4f},  S-measure_alpha05.\n'\
               .format(time.time()-start_time, self.dataset, self.method, mae, max_f, mean_f, adp_f, max_e, mean_e, adp_e, s_alpha05)
    ...
    def Eval_Fmeasure(self):
        ...
        with torch.no_grad():
            ...
            recall_avg /= img_num
            avg_f /= img_num
            pr_array = np.hstack((prec_avg.detach().cpu().numpy().reshape(-1, 1),
                                 recall_avg.detach().cpu().numpy().reshape(-1, 1)))
